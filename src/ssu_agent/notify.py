...
헤르메스봇과 **같은 봇 토큰·같은 채팅방**을 쓴다. 봇 인스턴스는 공유하고
코드만 분리하는 것이 이 설계다(핸드오프 §4).

그래서 이 모듈은 sendMessage 만 호출한다. getUpdates 를 부르는 순간
헤르메스봇의 폴링과 업데이트 경합이 난다. 양방향(버튼 체크)이 폐기안인
이유가 이것이고, 쓰기 경로는 LMS 하나로 유지한다.
