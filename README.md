...
| **퀴즈 매핑** | ✅ **2026-09-14.** 퀴즈·과제도 강의처럼 접는다. `ambiguous_canvas` 오탐 제거 — 4차산업 1주차 퀴즈가 영영 ⬜ 였다 |
...
| `sync.py` | 모듈·lessons·attendance_items·제출현황·공지 수집 → `state/snapshot.json` |
...
| `study_cli.py` | Canvas 상태 → vault. `study.py` 를 서브프로세스로 (직접 쓰기 없음). **주차×유형으로 접는다** — 강의·퀴즈·과제 모두 |
...
| `status.py` | 요약 현황만 센다(장부·손 업로드). 네트워크·LLM 안 탐 — `_targets` 를 그대로 세므로 화면과 실행이 안 어긋난다 |
| `refresh.py` | **수집→vault→자료** 세 단계. **코코봇의 입구** — 순서·중단·보고를 여기가 진다. **요약은 여기 없다** — LLM 을 안 부른다 |
| `lock.py` | `state/{name}.lock` flock. 크론과 수동 실행이 겹쳐도 뒤에 온 쪽이 조용히 빠진다(에러 아님) |
...
```
"최신 LMS 업데이트해줘"  → ssu-agent refresh          (이제 돈을 안 쓴다)
"요약해줘"                → ssu-agent status → 골라서 → summarize --manual-only / --include-manual
```

```bash
./bin/ssu-agent refresh                  # 수집→vault→자료. LLM 0
./bin/ssu-agent refresh --dry-run        # 확인만
./bin/ssu-agent refresh --no-materials   # 자료 단계만 제외
```
...
vault 를 고치면 마감이 되돌아간다. 나머지 단계는 서로 독립이다. 401·연결 실패는
**은지가 읽을 문장으로 번역해서** 낸다.

🔴 **`refresh` 는 더 이상 LLM 을 부르지 않는다** (2026-09-06). 요약은 `summarize`
가 따로 진다 — `state/summarize.lock` 이 크론과 수동 실행의 충돌을 막는다.
...
| `com.eunzi.ssu-materials` | 매일 03:00 | `sync` → `materials` → `summarize` (**비용 발생**, `sources=("ledger",)` 고정 — 손 업로드는 안 본다) → 끝에 `status` 한 줄을 `cron.log` 에 남긴다 |
...
정찰 문서와 달랐던 것 5건. 자세한 건 핸드오프 §2.1 · §2.2 · §3.2.
**§2.2 는 2026-09-05 에 통째로 재정정됐다** — 1·4·5 번이 그 결과다.

1. **~~`attendance_items` 404 는 "안 봤다"~~ → 틀렸다. `id` 가 틀린 것이었다**
   (2026-09-05 재정정). 모듈 `external_url` 의
   `.../lecture_attendance/items/view/{id}` 는 과목을 복제하면 **옛 학기 id 가
   그대로 남는다.** LTI 뷰 페이지는 현재 id 로 바꿔치기해 렌더링하지만
   (`data-item_id`) API 는 안 해준다. 권위는 **`attendance_items/summary`** 다.
   근거 — `summary` 와 모듈 URL id 의 차집합 크기가 과목별 404 개수와 정확히
   일치했다: 확장현실 15, 정치철학 53, 4차산업 48, 한반도 35, 선형대수·3code 0.
   피해: 확장현실 1주차는 시청 완료 + 대면 출석 + 퀴즈까지 다 돼 있었는데
   한 건도 vault 에 안 올라갔다.
2. **`sessionless_launch` URL 을 직접 조립하면 500** (이중 인코딩).
   모듈 아이템의 `url` 필드를 그대로 쓴다.
3. **알림 전송을 걷어냈다.** iMac 에 이미 봇이 있다. 판단은 여기, 전달은 저기.
4. **`progress` 는 초다** (2026-09-05). 확장현실 1주차 progress 2307.42 ·
   duration 2307.43 · last_at 2307.43. 퍼센트로 읽던 코드는 `dur * 23.07` 을
   만들어 **부분 시청 영상을 전부 '다 봤다'로** 떨어뜨렸다 — 완료 항목은
   단락돼서 그동안 안 드러났다.
5. **퀴즈·과제는 제출 여부를 조회조차 안 하고 있었다** (2026-09-05).
   구조적으로 영원히 미완료였다. Canvas `students/submissions` 로 본다.
   단 `practice_quiz` 는 assignment 가 없어 거기 안 나온다 —
   `quizzes/{id}/submission` 을 따로 봐야 한다 (확장현실 1주차 퀴즈가 그 경우).
