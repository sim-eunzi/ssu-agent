...
- **깨진 `meta.json` + "아직 한 번도 요약된 적 없는 장부 PDF"의 좁은 이중 과금 창** —
  `.progress` 를 대체 장부로 쓰는 규칙(§3)은 **이미 요약된** 수집본만 지킨다. 그 주차에
  meta.json 이 깨진 시점에 **아직 한 번도 요약을 안 돈 장부 PDF**가 있으면 `.progress`
  에도 근거가 없어 `manual-{파일명}` 키로 새로 요약된다 — 나중에 meta 를 복구해 장부
  경로로 다시 돌리면 같은 문서가 두 번 청구된다(지연된 이중 과금). Ruling(구현 중,
  현 동작 유지): (1) 발생 조건이 3중이다 — meta.json 손상 + 그 주차에 미요약 장부 PDF
  존재 + 그 창에서 사람이 `--include-manual`/`--manual-only` 실행. (2) 대안(깨진 주차
  에서 manual 스캔을 아예 끊기)은 "화면엔 보이는데 요약만 조용히 사라진다"는 원래
  문제를 되살린다. (3) 대상 선정 규칙을 여기서 바꾸면 `status` 계수 근거까지 흔들린다.
  비용: 위 3중 조건이 실제로 겹치면 문서 하나치 요약비가 두 번 나간다(수 센트~수십 센트)
- **manual↔ledger 별칭이 이중 청구는 막아도 내용까지 맞춰주지 않는다** —
  손으로 올린 `강의.pdf` 를 먼저 요약해 `manual-강의.pdf.json` 이 `done` 이 된 뒤,
  `materials.run` 이 같은 이름으로 파일을 덮어쓰고 새 숫자 content_id 로 등록하면
  `load_progress_for` 가 별칭을 찾아 "이미 됐다"고 건너뛴다 — 맞는 판단이다, 문서를
  두 번 청구하지 않는다. 하지만 저장된 `summary.md` 는 **덮어써지기 전 리비전**에서
  나온 것이고, LMS 가 실제로 발행한 새 리비전과 내용이 다를 수 있다. content_id 별칭은
  "같은 파일명"만 보지 "같은 내용"은 안 본다 — 이 차이는 조용히 남는다. 고치려면 원문
  해시로 리비전을 구분해야 하는데, 그건 「장부 밖에 둔다」는 손 업로드 계약과
  `materials.py` 의 덮어쓰기 동작 양쪽을 건드린다. `materials.py` 는 이 스펙의 범위
  밖이라 여기서는 손대지 않는다 — 알고 남긴다
